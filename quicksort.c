...
{
    int p;
    //printf("            [quicksort serial: lo= %d hi=%d]\n",lo,hi);
    if(lo >= hi)
        return;
...
{
    //printf("je rentre dans quicksort\n");
    ...
    p = partition(a, lo, hi);
    //printf("            [quicksort: lo= %d hi=%d je fais spawn 1]\n",lo,hi);
    rc = sched_spawn(quicksort, new_args(a, lo, p), s);
    assert(rc >= 0);
    //printf("            [quicksort: lo= %d hi=%d je fais spawn 2]\n",lo,hi);
...
int main(int argc, char **argv)
{
    ...
    if(serial) {
        ...
        rc = sched_init(nthreads, (n + 127) / 128, quicksort, new_args(a, 0, n - 1));
        assert(rc >= 0);
    }
